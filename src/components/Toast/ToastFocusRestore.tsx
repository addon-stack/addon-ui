...
export function ToastFocusRestore({viewport}: {viewport: HTMLOListElement | null}) {
    ...
    useLayoutEffect(() => {
        ...
        if (!marker || !toast || !viewport || !getShadowRoot(toast)) {
            return;
        }
        ...
        return () => {
            ...
            if (!containsComposed(toast, focused)) {
                return;
            }
            ...
            queueMicrotask(() => {
                if (marker.isConnected || !viewport.isConnected) {
                    return;
                }

                if (toast.isConnected && toast.dataset.state !== "closed") {
                    return;
                }
