...
export function useFloatingFocus({ref: layerRef, node, layer}: FloatingLayerHandle) {
    ...
    useEffect(() => {
        if (!node || !layer.active || !layer.modal || !getShadowRoot(node)) return;
        const doc = node.ownerDocument;
        const root = getShadowRoot(node)!;
        let restoring = false;
        const restore = () => {
            if (restoring || !isLiveLayer(layer) || !isTopModal(layer)) return;
        ...
        const observer = new MutationObserver(() => {
            if (lastFocus.current && !lastFocus.current.isConnected) restore();
        });
    ...
    const onOpenAutoFocus = (event: Event) => {
        if (event.defaultPrevented || !node || !getShadowRoot(node)) return;
        event.preventDefault();
        // An already focused nested layer owns its autofocus.
        if (!ownsFocus(layer, getActiveElement(node))) focusElement(getTabbableElements(node)[0] ?? node);
    ...
    const onCloseAutoFocus = (event: Event) => {
        ...
        event.preventDefault();
        if (opener.current?.isConnected) focusElement(opener.current);
    };
    ...
    const onKeyDown = (event: KeyboardEvent<HTMLDivElement>) => {
        if (
            ...
            return;
        const focused = getActiveElement(node);
        if (getOwnedLayers(layer).some(candidate => candidate !== layer && containsComposed(candidate.node!, focused)))
            return;
