...
import {verifyCustomizationBuild} from "../support/verify-customization-build.mjs";

const execute = promisify(execFile);

// Both browser builds use each fixture's virtual-module directory. Keep them sequential.
for (const app of ["shadow-ui", "customization"]) {
    for (const engine of ["chrome", "firefox"]) {
        const root = fixtureRoot;

        test(`${app} ${engine}: extension build, CSS delivery and declarations`, async ({}, testInfo) => {
            ...
            try {
                build = await execute(
                    ...
                    [path.join(root, "node_modules/adnbn/bin/adnbn.js"), "build", "-b", engine,
                        "-c", app === "customization" ? "customization.config.ts" : "adnbn.config.ts"],
                    {
                        cwd: root,
                        timeout: 90_000,
            ...
            const manifest = JSON.parse(
                await fs.readFile(path.join(buildDirectory(root, engine, app), "manifest.json"), "utf8")
            );
            ...
            await testInfo.attach("build-verification", {
                body: JSON.stringify(
                    app === "customization" ? verifyCustomizationBuild(root, engine) : verifyBuild(root, engine)
                ),
                contentType: "application/json",
            });

            // Declarations are independent of the browser target: check once per fixture.
            if (engine === "chrome" && app === "shadow-ui") {
