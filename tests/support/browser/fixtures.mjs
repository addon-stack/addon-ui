import {test as base, expect, chromium, firefox} from "@playwright/test";
import fs from "node:fs/promises";
import path from "node:path";
import os from "node:os";
import http from "node:http";
import {fixtureRoot, buildDirectory} from "../paths.mjs";
    ...
    extension: [
        async ({engine, siteURL}, use) => {
            ...
            try {
                ...
                if (debugPort) await installFirefoxAddon(debugPort, directory);
        ...
        async ({extension}, use, testInfo) => {
            ...
            const observe = page => {
                ...
                const onConsole = message => {
                    if (message.type() === "error") messages.push({type: "console", message: message.text()});
                };
            ...
            try {
                ...
                const unexpected = messages.filter(
                    item =>
                        item.type !== "console" || !item.message.startsWith("`DialogContent` requires a `DialogTitle`")
                );
                runtimeFailed = unexpected.length > 0;
                expect(unexpected, "Unexpected extension runtime errors").toEqual([]);
            } finally {
                ...
                for (const dispose of observed.values()) dispose();
                // Keep the initial tab alive so Firefox does not close its last window between tests.
                for (const page of context.pages()) if (!retainedPages.has(page)) await page.close();
            }
        },
        {auto: true},
    ],
