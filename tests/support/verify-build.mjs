...
export function verifyBuild(root, engine) {
    ...
    for (const asset of ["style-root", "style-nested"]) {
        const file = war.find(file => file.endsWith(".svg") &&
            fs.readFileSync(path.join(out, file), "utf8").includes(`virtual-${asset}`));

        assert.ok(file, `${asset} must be web accessible`);
        assert.ok(fs.existsSync(path.join(out, file)), `${asset} must be emitted`);
        assert.ok(css.some(text => text.includes(path.basename(file))), `${asset} must be referenced in isolated CSS`);
    }
